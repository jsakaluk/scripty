#' Calculates weighted unsigned difference in expected indicator scores
#'
...
get_wudi <- function(dat, fit, nodewidth = 0.01, lowerLV = -5, upperLV = 5){
  ...
  intnone <- nogroupints(nogroupfit)
  intercept1 = int1$est
  intercept2 = int2$est
  ...
  intfac1 <- grouplvmean(fit, source = "1")
  sdfac1 <- grouplvsd(fit, source = "1")
  fmean1 = intfac1$est
  fsd1 = sdfac1$est
